...
function createEnemy($container, x, y){
  ...
  const enemy_cooldown = Math.floor(Math.random()*100);// velocidad de disparos de los enemy
  const enemy = {x, y, $enemy, enemy_cooldown}
  STATE.enemies.push(enemy);
  setSize($enemy, STATE.enemy_width);
  setPosition($enemy, x, y)
}
//enemi velocidad,cooldown
function updateEnemies($container){
  const dx = Math.sin(Date.now()/1000)*40; //velocidad del enemigo
...
function updatePlayer(){
  if(STATE.move_left){
    ...
    STATE.cooldown = 20;//velocidad de disparo
  }
...
//funcion lazer para el gameover
...
// inicio juego, creacion cointainer
const $container = document.querySelector(".main");
createPlayer($container);
createEnemies($container);

// evento tecla arriba y abajo
